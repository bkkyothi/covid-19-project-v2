...
function getContinentColor(cases: number): string {
  if (cases > 50000000) return '#7f1d1d'; // very high
  if (cases > 25000000) return '#b91c1c';
  if (cases > 10000000) return '#c2410c';
  if (cases > 5000000) return '#ea580c';
  if (cases > 1000000) return '#d97706';
  return '#65a30d'; // low
}
...
async function initMap() {
  ...
  try {
    ...
    const style = (feature: any) => {
      ...
      const fillColor = data ? getContinentColor(data.cases) : '#cccccc';
    ...
    const onEachFeature = (feature: any, layer: any) => {
      ...
      layer.on({
        ...
        mouseout: (e: any) => {
          // If selected, don't reset fully?
          // We rely on style function update or reset to original style
          // Leaflet resetStyle resets to the style provided in options
          // But our options.style is a function that checks selected state.
          // However, geoJsonLayer.resetStyle(e.target) re-evaluates the style function?
          // Yes, resetStyle re-calls the style function.
